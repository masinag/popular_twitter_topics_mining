...
def get_frequent_items(data, s):
    min_freq = int(s * len(data))
    log(f"Looking for items appearing more than {min_freq} times out of {data.shape[0]}")
    ...
    for _, tokens in data.iteritems():
        for token in tokens:
            if not token in token_id:
                token_id[token] = i
                id_token.append(token)
                curr_occ.append(1)
                i += 1
            else:
                curr_occ[token_id[token]] += 1 

    frequencies = [{(i, ) for i, occ in enumerate(curr_occ) if occ >= min_freq}]
    log(f"Got {len(frequencies[-1])} frequent itemsets of size 1")
    
    # map each token to its id
    idata = data.apply(lambda l : list(map(lambda x : token_id[x], l)))
    ...
    while len(frequencies[-1]) > 1:
        ...
        for _, tokens in idata.iteritems():
        ...
        frequencies.append({token for token, occ in curr_occ.items() if occ >= min_freq})
        log(f"Got {len(frequencies[-1])} frequent itemsets of size {n}")
        n += 1
        curr_occ.clear()

    return (tuple(map(lambda y : id_token[y], y)) for y in functools.reduce(operator.iconcat, frequencies, []))
...
if __name__ == "__main__":
    ...
    s = 0.01 # fraction of tweets per period of time
    p = 0.04 # fraction of periods of time considered
